...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="mt-3 font-serif text-4xl font-bold text-[#f0ece4]">Create Barber</h1>
    </div>

    <form method="POST" action="{{ route('admin.barbers.store') }}" x-data="{ photoUrl: @js(old('photo_url')) }" class="grid gap-6 lg:grid-cols-[1fr_320px]">
        @csrf
        <section class="border border-[#2a2a2a] bg-[#161616] p-6">
            ...
            <label class="mt-4 block">
                ...
                <input name="specialty" value="{{ old('specialty') }}" class="mt-2 h-12 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">
                @error('specialty') <span class="mt-1 block text-sm text-[#b94a4a]">{{ $message }}</span> @enderror
            </label>

            <label class="mt-4 block">
                <span class="text-sm text-[#f0ece4]">Bio</span>
                <textarea name="bio" rows="6" class="mt-2 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 py-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">{{ old('bio') }}</textarea>
            ...
            <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                <button type="submit" class="h-12 bg-[#c9a84c] px-5 font-bold uppercase tracking-wide text-[#0e0e0e] transition duration-200 hover:bg-[#8a6f32]">Save Barber</button>
