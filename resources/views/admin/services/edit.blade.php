...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="mt-3 font-serif text-4xl font-bold text-[#f0ece4]">Edit Service</h1>
    </div>

    <form method="POST" action="{{ route('admin.services.update', $service) }}" x-data="{ imageUrl: @js(old('image_url', $service->image_url)) }" class="grid gap-6 lg:grid-cols-[1fr_320px]">
        @csrf
        @method('PATCH')
        <section class="border border-[#2a2a2a] bg-[#161616] p-6">
            <label class="block">
                <span class="text-sm text-[#f0ece4]">Name</span>
                <input name="name" value="{{ old('name', $service->name) }}" required class="mt-2 h-12 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">
            ...
            <label class="mt-4 block">
                ...
                <textarea name="description" rows="5" class="mt-2 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 py-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">{{ old('description', $service->description) }}</textarea>
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                <label class="block">
                    ...
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="5" required class="mt-2 h-12 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">
                ...
                <label class="block">
                    ...
                    <input type="number" name="price" value="{{ old('price', $service->price) }}" min="0" step="0.01" required class="mt-2 h-12 w-full border border-[#2a2a2a] bg-[#1f1f1f] px-3 text-[#f0ece4] outline-none focus:border-[#c9a84c]">
            ...
            <label class="mt-5 flex items-center gap-3 text-sm text-[#f0ece4]/75">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $service->is_active)) class="h-4 w-4">
                Active
            </label>

            <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                <button type="submit" class="h-12 bg-[#c9a84c] px-5 font-bold uppercase tracking-wide text-[#0e0e0e] transition duration-200 hover:bg-[#8a6f32]">Update Service</button>
